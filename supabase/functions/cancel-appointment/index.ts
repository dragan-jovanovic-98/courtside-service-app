...
serve(async (req) => {
  ...
  try {
    ...
    if (!appointment_id) {
      return errorResponse("appointment_id is required");
    }

    // Validate appointment belongs to org and get current notes
    ...
    if (reason) {
      ...
      updatePayload.notes = `${existingNotes}Cancelled: ${reason}`;
    }
    ...
    if (n8nUrl) {
      await fetch(`${n8nUrl}/appointment-cancelled`, {
        method: "POST",
